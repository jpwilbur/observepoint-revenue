...
6. **Emit the output contract** (`{rollup, per_domain[]}`) + the transparency line on discounted spirals. `size_site_census` only itemizes the top ~40 hostnames — on bigger accounts, add a single labeled **tail-aggregate row** so `per_domain` still sums to the anchor (see the reference). For each itemized domain, pull ~5–6 real sample pages into `url_samples` via the links grid (see the reference's `url_samples` recipe — clean pages, never fabricated). Then build the evidence workbook: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/derive-page-count/scripts/build_evidence_appendix.py <perdomain.json> <out.xlsx>` — pass a `usage` object too (from size-and-price: consent states, cadence layers, annual scans, recommended contract) to include the **Annual Usage Breakdown** sheet.
